...
use Yii;
use common\models\academicranks\ariStaffPosition;
use common\models\academicranks\StaffHistory;
use common\models\academicranks\ariPosition;
use common\models\academicranks\ariStaffAcademic;
use common\models\academicranks\ariAcademic;
use common\models\academicranks\ariStaffStatus;
use common\models\academicranks\ariEducation;
use common\models\academicranks\ariStaffEducate;
...
class ariStaff extends Staff {
    ...
    public function Position($citizen_id=NULL) {
        $subQuery = ariStaffPosition::findOne(['citizen_id' => $citizen_id]);
        $position_id=$subQuery->position_id;
        $query=ariPosition::findOne(['position_id'=>$position_id]);
    ...
    public function Positionsub($citizen_id=NULL) {
        ...
        $query=ariPositionSubtype::findOne(['position_subtype_id'=>$positionsub_id]);
        
        return $query->position_subtype;
    }
    public function Academic($citizen_id=NULL) {
        //$position_id=4;
        $SubQuery = ariStaffPosition::find()->select('position_id')->where(['citizen_id'=>$citizen_id]);
        if($SubQuery!=4){
           return "สายสนับสนุน";
        }
    ...
    {
        $query = ariStaffPosition::findOne(['citizen_id' => $citizen_id]);
            //    ->select(['status_date']);
       // Yii::$app->thaiFormatter->locale = 'th_TH';
        $start_work=$query->date_work;
       // Yii::$app->thaiFormatter->locale = 'th-TH';
        $dmy= Yii::$app->Formatter->asDate($start_work, 'php:Y-m-d'); 
        $d = new ndate();
...
//        }
    ...
    public function Status($citizen_id=NULL)
    {
        ...
        $query=ariStaffStatus::findOne(['status_id'=>$status_id]);
        return $query->title;
    }
    public function TotalTimeWork($citizen_id=NULL)
    {
       ...
       // Yii::$app->thaiFormatter->locale = 'th_TH';
        ...
        if([$todayM2, $d31]==TRUE){ $subD=31; }
        else if([$todayM2, $d30]==TRUE){ $subD=30; }
        else if([$todayM2, $d28]==TRUE){ if($LeapYear==1) { $subD=29; } else { $subD=28; } }
        ...
        else if(($todayY>$bY)&&($todayM<$bM)&&($todayD==$bD)) { $aY2=$todayY-$bY-1; $aM2=12-($bM-$todayM); $aD2=0; } 
     ...
     public function counterSeven($citizen_id)
    {
        ...
        if ($citizenid==""){
        $startwork=ariStaffPosition::findOne(['citizen_id' => $citizen_id]);
        ...
        $level=ariStaffEducate::find()->where(['citizen_id'=>$citizen_id])
                    ->one();
...
if($level_id >= 2){       
 ...
 if($authorise_date){  
  ...
  // return ($y.''.$m.''.$d);
   ...
   if($academic_id=3){
        $subQuery = ariStaffHistory::findOne(['citizen_id' => $citizen_id]);
        $status_id=$subQuery->status_id;
        if($status_id=4){
            $start_study = ariEducation::find()->where(['citizen_id' => $citizen_id])
                ->one();
    ...
    //   $st = mktime(1, 1, 1, $stmonth+$m, $stday+$d, $styear+$y);
      ...
      //  Yii::$app->thaiFormatter->locale = 'th-TH';
       ...
       // return $dmy;    
        ...
        $status=ariStaffHistory::find()->where(['citizen_id'=>$citizen_id])
                    ->one();
...
//            $stid=1;
      //  if($stid==1){
           ...
           $querys = ariStaffPosition::findOne(['citizen_id' => $citizen_id]);
     ...
     //   $timStmp2 = mktime(0,0,0,$bdate[1],$bdate[2],$bdate[0]);
      ...
      //  $levelid=1;//ข้อมูลจำลอง
            ...
            //   $timStmp1 = mktime(0,0,0,$studydate[1],$studydate[2],$studydate[0]);
                ...
                $todayM = $studydate[1];
                $todayD = $studydate[2];
                $query=ariStaffPosition::find()->where(['citizen_id'=>$citizen_id])
                    ->one();
                $start_work=$query->date_work;
                $bdate=explode("-",$start_work);
              //  $timStmp2 = mktime(0,0,0,$bdate[1],$bdate[2],$bdate[0]);
        ...
        //        $LeapYear=date("L"); // 1 = leap year Feb has 29 day
                ...
                $startdate=ariStaffPosition::find()->where(['citizen_id'=>$citizen_id])
                   ->orderBy(date_work)
                   ->one();
